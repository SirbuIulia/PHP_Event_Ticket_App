...
if (isset($_POST['submit']))
{
    ...
    $domeniu = htmlentities($_POST['domeniu'], ENT_QUOTES);
    $id_evt = htmlentities($_POST['id_evt'], ENT_QUOTES);

    if ($nume == '' || $prenume == '' || $nr_tel == '' || $email == '' || $domeniu == '' || $id_evt == '' )
    {
        $error = 'ERROR: Campuri goale!';
    }
    else
    {
        if ($stmt = $mysqli->prepare("INSERT into speakeri (Nume, Prenume, Nr_Tel, Email, Domeniu, ID_EVT) VALUES (?, ?, ?, ?, ?, ?)"))
        {
            $stmt->bind_param("ssssss", $nume, $prenume, $nr_tel, $email, $domeniu, $id_evt);
            $stmt->execute();
            $stmt->close();
        }
        else
...
<html>
<head>
    <title><?php echo "Inserare speaker"; ?></title>
...
<body>
<h1><?php echo "Inserare speaker"; ?></h1>
<?php if ($error != '') {
...
<form action="" method="post">
    <div>
        ...
        <strong>Domeniu: </strong> <input type="text" name="domeniu" value=""/><br/>
